...
def bars(data,th_tick,th_volume,th_dollar,index='timestamp',col_price='close',col_volume='volume'):
    ...
    volume = pd.DataFrame(columns=[col_price]) 
    volume.index.names = [index]
    
    dollar = pd.DataFrame(columns=[col_price]) 
    dollar.index.names = [index]
    ...
    for i in range(len(data.index)):
        ...
        if(cnt_v >= th_volume):
            cnt_v=0      
            tmp = data.iloc[i]                           
            volume.loc[data.index[i]] = [tmp[0]]
        
        
        if(cnt_d >= th_dollar):
            cnt_d=0      
            tmp = data.iloc[i]                           
            dollar.loc[data.index[i]] = [tmp[0]]
